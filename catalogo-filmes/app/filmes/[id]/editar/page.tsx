...
export default function EditarFilmePage() {
  ...
  const loadData = async () => {
    try {
      setLoading(true);
      const [filmeData, generosData, diretoresData] = await Promise.all([
        filmesAPI.get(Number(params.id)),
        generosAPI.list(),
        diretoresAPI.list()
      ]);
      ...
      setFormData({
        ...
        ano_publicacao: filmeData.ano_publicacao.toString(),
        sinopse: filmeData.sinopse || '',
        duracao: filmeData.duracao || '',
        poster: null,
        generos: filmeData.generos?.map((g: Genero) => g.id) || [],
        diretores: filmeData.diretores?.map((d: Diretor) => d.id) || []
      });
    } catch (err) {
      setErrors({ general: 'Erro ao carregar filme' });
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      await filmesAPI.update(Number(params.id), submitData);
      window.location.href = `/filmes/${params.id}`;
    } catch (error: any) {
      setErrors({ general: 'Erro ao atualizar filme. Tente novamente.' });
    } finally {
  ...
  if (!filme) {
    return (
      <div className="container py-5">
        <div className="alert alert-danger">Filme não encontrado</div>
      </div>
    );
  }
          ...
          <div className="row justify-content-center">
            <div className="col-lg-8">
              ...
              <p className="lead mb-4">
                Atualize as informações do filme no catálogo
              </p>
            </div>
          </div>
        </div>
                  ...
                  <form onSubmit={handleSubmit} encType="multipart/form-data">
                    ...
                    <div className="row">
                      ...
                      <div className="col-md-8 mb-4">
                        ...
                        {filme.poster && !posterFileName && (
                          <div className="mb-3">
                            <div className="card border-0 shadow-sm" style={{ borderRadius: '10px' }}>
                              <div className="card-body p-3">
                                <div className="row align-items-center">
                                  <div className="col-auto">
                                    <img src={filme.poster} alt="Poster atual" className="img-thumbnail" style={{ width: '80px', height: '120px', objectFit: 'cover' }} />
                                  </div>
                                  <div className="col">
                                    <h6 className="mb-1">Poster Atual</h6>
                                    <p className="text-muted mb-0 small">Imagem carregada com sucesso</p>
                                  </div>
                                </div>
                              </div>
                            </div>
                        ...
                        <div className="upload-area" onClick={() => document.getElementById('poster')?.click()} style={{ border: '2px dashed #e9ecef', borderRadius: '10px', padding: '40px 20px', textAlign: 'center', cursor: 'pointer', background: '#f8f9fa', transition: 'all 0.3s ease' }}>
                          <div className="upload-content">
                            ...
                            <h6 className="mb-2">{filme.poster ? 'Alterar Poster' : 'Adicionar Poster'}</h6>
                    ...
                    <div className="d-flex justify-content-between align-items-center mt-5">
                      <Link href={`/filmes/${params.id}`} className="btn btn-outline-secondary btn-lg px-4 py-2" style={{ borderRadius: '25px' }}>
